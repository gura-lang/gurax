...
bool Value_wxTimer::EntityT::Start(int milliseconds, bool oneShot)
{
	...
	do {
		...
		if (!args_gurax.FeedValue(core_gurax.GetProcessor(), new Gurax::Value_Number(milliseconds))) {
			Util::ExitMainLoop();
			break;
		}
		if (!args_gurax.FeedValue(core_gurax.GetProcessor(), new Gurax::Value_Bool(oneShot))) {
			Util::ExitMainLoop();
			break;
		}
		// Evaluation
