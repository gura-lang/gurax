...
{
	String str;
	str.Format("Value(%s)", GetValue().ToString(StringStyle::SymbolQuoted_NilVisible).c_str());
...
String PUnit_ForEach<discardValueFlag>::ToString(const StringStyle& ss, int seqIdOffset) const
{
	...
	str.Format("ForEach(offsetToIterator=%zu,branchDest=%s, decls=[%s])", GetOffset(),
			   MakeSeqIdString(GetPUnitBranchDest(), seqIdOffset).c_str(),
			   GetDeclArgOwner().ToString(StringStyle().SetSymbolQuoted().SetCram()).c_str());
	AppendInfoToString(str, ss);
	return str;
}
...
String PUnit_ArgSlot_Value<discardValueFlag>::ToString(const StringStyle& ss, int seqIdOffset) const
{
	...
	str.Format(",%s,cont=%s)",
				GetValue().ToString(StringStyle::SymbolQuoted_NilVisible).c_str(),
				MakeSeqIdString(_GetPUnitCont(), seqIdOffset).c_str());
	AppendInfoToString(str, ss);
	return str;
}
...
{
	String str;
	str.Format("FailCatch(%s)", GetValue()->ToString(StringStyle::SymbolQuoted_NilVisible).c_str());
	AppendInfoToString(str, ss);
	return str;
}
