...
{
	String str;
	str.Format("Value(%s)", GetValue().ToString(StringStyle::Quoted_NilVisible).c_str());
...
String PUnit_ArgSlot_Value<discardValueFlag>::ToString(const StringStyle& ss, int seqIdOffset) const
{
	...
	str.Format(",%s,cont=%s)",
				GetValue().ToString(StringStyle::Quoted_NilVisible).c_str(),
				MakeSeqIdString(_GetPUnitCont(), seqIdOffset).c_str());
	AppendInfoToString(str, ss);
	return str;
}
...
{
	String str;
	str.Format("FailCatch(%s)", GetValue()->ToString(StringStyle::Quoted_NilVisible).c_str());
	AppendInfoToString(str, ss);
	return str;
}
