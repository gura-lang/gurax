...
Gurax_ImplementStatement(cond)
{
	...
	if (pExprFalse) {	// cond (cond, exprTrue, exprFalse)
		...
		composer.Add_JumpIfNot(false, &exprCaller);						// []
...
Gurax_ImplementStatement(try2)
{
	...
	for (Expr_Caller* pExpr = exprCaller.GetExprTrailer(); pExpr; ) {
		Expr_Caller* pExprNext = pExpr->GetExprTrailer();
		if (pExpr->IsStatement(Gurax_Symbol(catch_))) {
			if (pExpr->GetExprCdrFirst()) {
	...
	for (Expr_Caller* pExprCatch : exprsCatch) {
		...
		composer.Add_JumpIfNoCatch(pExprCatch);							// [Error] or []
		if (pDeclArg) {
	...
	if (iterFlag || xiterFlag) {
		...
		if (declArgsOfBlock.empty()) {
			if (createListFlag) {
				...
				PUnit* pPUnitOfBranch = composer.PeekPUnitCont();
				composer.Add_JumpIfNot(false, &exprCaller);						// [List]
				composer.BeginRepeaterBlock(pPUnitOfLoop, pPUnitOfBranch, pPUnitOfBreak);
				...
				PUnit* pPUnitOfBranch = composer.PeekPUnitCont();
				composer.Add_JumpIfNot(false, &exprCaller);						// [Last]
			...
			if (createListFlag) {
				...
				PUnit* pPUnitOfBranch = composer.PeekPUnitCont();
				composer.Add_JumpIfNot(false, &exprCaller);						// [Iterator List]
				...
				PUnit* pPUnitOfBranch = composer.PeekPUnitCont();
				composer.Add_JumpIfNot(false, &exprCaller);						// [Iterator Last]
